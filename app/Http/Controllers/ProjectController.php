...
class ProjectController extends Controller
{
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                $actions = '<div class="edit-delete-action">';
                    ...
                    $actions .= '<a href="javascript:;" onclick="remove_row(\'' . url('admin/projects/' . $row->id) . '\')" data-bs-toggle="modal" data-bs-target="#delete-modal" class="p-2" title="Delete">
    ...
    public function store(Request $request)
    {
        try {
            ...
            $after_avatar = "";
            if ($request->hasFile('after_image') && $request->file("after_image")->isValid()) {
    ...
    public function update(Request $request,$id)
    {
        try {
            ...
            if ($request->hasFile('image') && $request->file("image")->isValid()) {
                ...
                // delete old image
                if($post["old_avatar"] != "" && file_exists(public_path('uploads/project/'.$post["old_avatar"]))) {
            ...
            return response()->json(['success' => true,'message' => "Project edited successfully."], 200);
        } catch (\Exception $e) {
            return response()->json(['success' => false,'message' => $e->getMessage()], 200);
        }
    }
    ...
    {
        Project::destroy($id);
        return response()->json(['success' => true,'message' => "Project removed successfully."], 200);
    }
}
