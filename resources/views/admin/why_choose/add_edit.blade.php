...
<form action="{{ is_null($reason) ? url('admin/why_chooses') : url('admin/why_chooses/'.$reason->id) }}" method="POST" enctype="multipart/form-data" id="mainForm">
    @csrf
    @if(!is_null($reason))
        <input type="hidden" name="_method" value="PUT" />
    @endif
    <input type="hidden" class="form-control" name="old_file" value="{{ is_null($reason) ? '' : $reason->file }}" />
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4>{{ is_null($reason) ? "New" : "Edit" }} Reason</h4>
                </div>
                <div class="card-body profile-body">
                    <div class="row">
                        <div class="col-lg-9 mb-3">
                            <label class="form-label">Title<span class="text-danger ms-1">*</span></label>
                            <input type="text" class="form-control" name="name" id="name" value="{{ is_null($reason) ? '' : $reason->name }}" autofocus />
                        </div>
                        <div class="col-lg-3 mb-3">
                            <label class="form-label">Status</label>
                            <select class="select" name="is_active" id="is_active">
                                <option value="1" {{ !is_null($reason) && $reason->is_active == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ !is_null($reason) && $reason->is_active == 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                    </div>
                    <div class="text-end mt-2">
                        <button type="submit" class="btn btn-primary">SUBMIT</button>
                        <a href="{{ url('admin/why_chooses') }}" class="btn btn-secondary" id="backBtn">Back</a>
                    </div>
                </div>
            </div>
        </div>
